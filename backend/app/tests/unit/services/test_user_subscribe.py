...
def test_user_subscribe_already_subscribed():
    ...
    subscription_data = SubscriptionRequest(city="TestCity", temperature_threshold=25.5)
    
    with patch('services.subscribe.is_subscribed', return_value=True):
        with pytest.raises(HTTPException) as exc:
            user_subscribe(db=mock_db, subscription=subscription_data, user_id=1)
...
def test_user_subscribe_success():
    mock_db = create_autospec(Session)
    subscription_data = SubscriptionRequest(city="kazan", temperature_threshold=25.5)
    ...
    with patch('services.subscribe.is_subscribed', return_value=False), \
         patch('services.subscribe.send_subscription_email.delay') as mock_send:

        result = user_subscribe(db=mock_db, subscription=subscription_data, user_id=1)
